...
TEST_F(WarmBootEventHandlerTest, ProcessFreezeNotificationSuccess) {
  EXPECT_CALL(mock_consumer_notifier_, WaitForNotificationAndPop)
      .WillOnce(DoAll(SetArgReferee<0>("freeze"), SetArgReferee<1>(""),
                      SetArgReferee<2>(std::vector<swss::FieldValueTuple>{}),
                      Return(true)));
  EXPECT_CALL(mock_p4runtime_,
              HandleWarmBootNotification(
                  Eq(swss::WarmStart::WarmBootNotification::kFreeze)))
      .WillOnce(Return(absl::OkStatus()));

  EXPECT_OK(warm_boot_events_.WaitForEventAndHandleWarmBootOperations());
}

TEST_F(WarmBootEventHandlerTest, ProcessUnfreezeNotificationSuccess) {
  EXPECT_CALL(mock_consumer_notifier_, WaitForNotificationAndPop)
      .WillOnce(DoAll(SetArgReferee<0>("unfreeze"), SetArgReferee<1>(""),
                      SetArgReferee<2>(std::vector<swss::FieldValueTuple>{}),
                      Return(true)));
  EXPECT_CALL(mock_p4runtime_,
              HandleWarmBootNotification(
                  Eq(swss::WarmStart::WarmBootNotification::kUnfreeze)))
      .WillOnce(Return(absl::OkStatus()));

  EXPECT_OK(warm_boot_events_.WaitForEventAndHandleWarmBootOperations());
}

TEST_F(WarmBootEventHandlerTest, UnknownNotificationFail) {
  EXPECT_CALL(mock_consumer_notifier_, WaitForNotificationAndPop)
      .WillOnce(DoAll(SetArgReferee<0>("unknown"), SetArgReferee<1>(""),
                      SetArgReferee<2>(std::vector<swss::FieldValueTuple>{}),
                      Return(true)));

  EXPECT_CALL(mock_p4runtime_, HandleWarmBootNotification).Times(0);

  EXPECT_THAT(
      warm_boot_events_.WaitForEventAndHandleWarmBootOperations(),
      StatusIs(absl::StatusCode::kUnknown,
               HasSubstr("Unknown warmboot notification unknown received")));
}

TEST_F(WarmBootEventHandlerTest, DoNotProcessWhenNotificationFails) {
  EXPECT_CALL(mock_consumer_notifier_, WaitForNotificationAndPop)
  ...
  EXPECT_THAT(
      ...
      StatusIs(
          ...
          HasSubstr("Warm-boot events failed waiting for a notification.")));
}
