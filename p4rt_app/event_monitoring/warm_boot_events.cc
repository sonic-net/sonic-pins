...
absl::Status WarmBootEventHandler::WaitForEventAndHandleWarmBootOperations() {
  ...
  const auto it = swss::WarmStart::warmBootNotificationReverseMap()->find(key);
  if (it == swss::WarmStart::warmBootNotificationReverseMap()->end()) {
    return gutil::UnknownErrorBuilder()
           << "Unknown warmboot notification " << key << " received";
  }
  swss::WarmStart::WarmBootNotification notification = it->second;

  return p4runtime_.HandleWarmBootNotification(notification);
}
