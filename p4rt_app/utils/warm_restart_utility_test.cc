...
TEST_F(WarmRestartUtilityTest, ShouldWaitForUnfreeze) {
  warm_boot_state_adapter_->SetWaitForUnfreeze(true);
  warm_boot_state_adapter_->SetOrchAgentWarmBootState(
      swss::WarmStart::WarmStartState::RECONCILED);
  EXPECT_TRUE(warm_restart_util_->ShouldWaitForGlobalUnfreeze());
  EXPECT_TRUE(warm_restart_util_->IsOrchAgentWarmBootReconciled());
  EXPECT_TRUE(warm_boot_state_adapter_->WaitForUnfreeze());

  warm_boot_state_adapter_->SetWaitForUnfreeze(false);
  warm_boot_state_adapter_->SetOrchAgentWarmBootState(
      swss::WarmStart::WarmStartState::RECONCILED);
  EXPECT_FALSE(warm_restart_util_->ShouldWaitForGlobalUnfreeze());
  EXPECT_TRUE(warm_restart_util_->IsOrchAgentWarmBootReconciled());
  EXPECT_FALSE(warm_boot_state_adapter_->WaitForUnfreeze());

  warm_boot_state_adapter_->SetWaitForUnfreeze(false);
  warm_boot_state_adapter_->SetOrchAgentWarmBootState(
      swss::WarmStart::WarmStartState::FAILED);
  EXPECT_FALSE(warm_restart_util_->ShouldWaitForGlobalUnfreeze());
  EXPECT_FALSE(warm_restart_util_->IsOrchAgentWarmBootReconciled());
  EXPECT_FALSE(warm_boot_state_adapter_->WaitForUnfreeze());
  ...
  EXPECT_FALSE(warm_restart_util_->ShouldWaitForGlobalUnfreeze());
  EXPECT_FALSE(
      warm_restart_util_->IsOrchAgentWarmBootReconciled(absl::Seconds(5)));
  EXPECT_FALSE(warm_boot_state_adapter_->WaitForUnfreeze());
}
...
TEST_F(WarmRestartUtilityTest, GetPortsFromConfigDb) {
  ...
  send_to_ingress_table_config_db_->set("SEND_TO_INGRESS_PORT",
                                        {{"value", {"NULL", "NULL"}}});
  expected_ports.push_back({"SEND_TO_INGRESS_PORT"});
  EXPECT_THAT(warm_restart_util_->GetPortsFromConfigDb(),
              UnorderedElementsAreArray(expected_ports));
}
}  // namespace
